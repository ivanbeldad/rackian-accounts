...
CHART_DIR="$BUILD_DIR/chart/rackian-auth"
GCS_PROTOCOL="gs"
GCS_BUCKET="rackian-cloud"

if [ $ENVIRONMENT = "production" ]; then
  RELEASE_NAME="rackian-auth"
  SECRET_PATH="$CHART_DIR/values.secret.yaml"
  GCS_SECRET="auth/values.secret.yaml"
  GCS_SECRET_PATH="$GCS_PROTOCOL://$GCS_BUCKET/$GCS_SECRET"
else
  RELEASE_NAME="rackian-auth-staging"
  SECRET_PATH="$CHART_DIR/values.secret.staging.yaml"
  GCS_SECRET="auth/values.secret.staging.yaml"
  GCS_SECRET_PATH="$GCS_PROTOCOL://$GCS_BUCKET/$GCS_SECRET"
fi
